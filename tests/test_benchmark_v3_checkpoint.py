from __future__ import annotations

import pytest

torch = pytest.importorskip("torch")

from norishio_lm.benchmark_v3_checkpoint import (
    checkpoint_file_sha256,
    expected_schedule_sha256,
    load_tournament_checkpoint,
    save_tournament_checkpoint,
    state_sha256,
)
from norishio_lm.benchmark_v3_model import build_model


class SizedModel(torch.nn.Module):
    def __init__(self, count: int) -> None:
        super().__init__()
        self.weight = torch.arange(count, dtype=torch.float32)


def _sha(char: str) -> str:
    return char * 64


def test_checkpoint_roundtrip_and_exclusive_publish(tmp_path):
    model = build_model("H0L0", seed=7)
    initial = state_sha256(model.state_dict())
    model.encoder.projection.weight.data.add_(1)
    path = tmp_path / "run.pt"
    record = save_tournament_checkpoint(
        path, model, arm="H0L0", seed=7,
        schedule_sha256=expected_schedule_sha256(7), initial_state_sha256=initial,
    )
    assert record["checkpoint_file_sha256"] == checkpoint_file_sha256(path)
    loaded = load_tournament_checkpoint(path, arm="H0L0", seed=7)
    assert state_sha256(loaded["state_dict"]) == record["final_state_sha256"]
    assert loaded["model"].training is False
    with pytest.raises(FileExistsError):
        save_tournament_checkpoint(
            path, model, arm="H0L0", seed=7,
            schedule_sha256=expected_schedule_sha256(7), initial_state_sha256=initial,
        )


def test_checkpoint_rejects_wrong_count_and_nonfinite_state(tmp_path):
    with pytest.raises(TypeError, match="frozen BenchmarkV3Model"):
        save_tournament_checkpoint(
            tmp_path / "wrong.pt", SizedModel(2), arm="H0L0", seed=7,
            schedule_sha256=expected_schedule_sha256(7), initial_state_sha256=_sha("b"),
        )
    model = build_model("H0L0", seed=7)
    model.encoder.embedding.weight.data[0, 0] = float("nan")
    with pytest.raises(ValueError, match="nonfinite"):
        save_tournament_checkpoint(
            tmp_path / "nan.pt", model, arm="H0L0", seed=7,
            schedule_sha256=expected_schedule_sha256(7),
            initial_state_sha256=state_sha256(build_model("H0L0", seed=7).state_dict()),
        )


def test_checkpoint_rejects_tampered_state_binding_and_schema(tmp_path):
    model = build_model("H0L0", seed=7)
    path = tmp_path / "run.pt"
    save_tournament_checkpoint(
        path, model, arm="H0L0", seed=7,
        schedule_sha256=expected_schedule_sha256(7),
        initial_state_sha256=state_sha256(model.state_dict()),
    )
    raw = torch.load(path, map_location="cpu", weights_only=True)
    raw["state_dict"]["encoder.embedding.weight"][0, 0] += 1
    torch.save(raw, path)
    with pytest.raises(ValueError, match="state digest"):
        load_tournament_checkpoint(path, arm="H0L0", seed=7)
    with pytest.raises(ValueError, match="metadata"):
        load_tournament_checkpoint(path, arm="H0L1", seed=7)


def test_checkpoint_rejects_wrong_schedule_and_initial_state(tmp_path):
    model = build_model("D_AUX", seed=17)
    initial = state_sha256(model.state_dict())
    with pytest.raises(ValueError, match="schedule"):
        save_tournament_checkpoint(
            tmp_path / "schedule.pt", model, arm="D_AUX", seed=17,
            schedule_sha256=_sha("a"), initial_state_sha256=initial,
        )
    with pytest.raises(ValueError, match="initial state"):
        save_tournament_checkpoint(
            tmp_path / "initial.pt", model, arm="D_AUX", seed=17,
            schedule_sha256=expected_schedule_sha256(17), initial_state_sha256=_sha("b"),
        )


def test_checkpoint_requires_exact_payload_keys_and_cpu_state(tmp_path):
    model = build_model("NO_INPUT", seed=29)
    path = tmp_path / "run.pt"
    save_tournament_checkpoint(
        path, model, arm="NO_INPUT", seed=29,
        schedule_sha256=expected_schedule_sha256(29),
        initial_state_sha256=state_sha256(model.state_dict()),
    )
    raw = torch.load(path, map_location="cpu", weights_only=True)
    raw["extra"] = True
    torch.save(raw, path)
    with pytest.raises(ValueError, match="keys"):
        load_tournament_checkpoint(path, arm="NO_INPUT", seed=29)
