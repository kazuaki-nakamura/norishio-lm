"""Deterministic scheduling and terminal records for benchmark-v3 execution."""
...
def batch_schedule(*, seed: int, train_rows: int = 384,
    ...
    if type(train_rows) is not int or train_rows != 384:
        raise ValueError("benchmark-v3 requires exactly 384 training rows")
...
def validate_terminal_record(root: str | os.PathLike[str], record: Mapping[str, Any]) -> None:
    ...
    if record.get("arm") not in ARM_COUNTS or record.get("seed") not in SEEDS:
        raise ValueError("unknown tournament arm or seed")
    failure_keys = {
...
def write_terminal_record(root: str | os.PathLike[str], record: Mapping[str, Any]) -> Path:
    ...
    _publish_exclusive(target, canonical_bytes(dict(record)) + b"\n")
    return target


def validate_terminal_runs_partial(records: Sequence[Mapping[str, Any]],
                                   config: Mapping[str, Any]) -> None:
    """Apply the full validator by filling missing keys with inert failures."""
...
def collect_terminal_records(root: str | os.PathLike[str]) -> list[dict[str, Any]]:
    ...
    for arm, seed in expected:
        value = json.loads(record_path(root, arm, seed).read_text(encoding="utf-8"))
...
__all__ = [
    ...
    "validate_terminal_runs_partial", "write_terminal_record",
]
