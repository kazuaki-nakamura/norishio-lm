...
worker 内では独立した実装・テスト・レビューを `gpt-5.6-luna` のサブエージェントに積極委譲する。
親は要件整理・担当ファイルの分割・統合・最終検証・GitHub 状態変更を担当する。
各委譲には対象 worktree、規則、範囲、担当ファイル、受け入れ条件、成果物を渡し、同じファイルの同時編集を避ける。
小さな修正や分離不能な作業は親が実行する。Luna を使えない場合はその事実を記録し、利用したと偽らない。
Issue / PR の結果には実際のモデル・委譲範囲・統合検証を記録する。サブエージェントは別 Issue の取得・新規タスク作成・自動化登録をしない。
